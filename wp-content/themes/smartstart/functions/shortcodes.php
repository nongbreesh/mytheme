...
function ss_framework_video_sc( $atts ) {
	...
	$output = "<video id='video-js-id-$post->ID' class='video-js vjs-default-skin' width='680' height='400' controls {$preload} {$autoplay} {$poster} data-setup='{}' data-aspect-ratio='{$aspect_ratio}'>
			{$mp4}
			{$webm}
			{$ogg}
			{$track}
...
/* ---------------------------------------------------------------------- */
	...
	function ss_framework_post_carousel_sc( $atts ) {
		...
		if( have_posts() ):
			...
			$output .= '<ul class="post-carousel clearfix">';
	...
	function ss_framework_projects_carousel_sc( $atts ) {
		...
		if( have_posts() ) {
			...
			$output .= '<ul class="projects-carousel clearfix">';
	...
	function ss_framework_slider_sc( $atts, $content = null ) {
		...
		if( have_posts() ) while ( have_posts() ) : the_post();
			...
			$output = '<section id="' . $post->post_name . '" class="ss-slider">';
				...
				$output .= '<div class="content">';
					...
					foreach ( $social_links as $social_link ) {
						...
						if( ss_framework_get_custom_field( 'ss_social_link_' . $social_link ) )
							$output .= '<li class="' . esc_attr( $social_link ) . '"><a href="' . $address . ss_framework_get_custom_field( 'ss_social_link_' . $social_link ) . '">' . esc_attr( ucfirst( $social_link ) ) . '</a></li>';
