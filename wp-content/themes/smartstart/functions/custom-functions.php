	...
	function ss_framework_post_meta() {
		...
		if( isset( $GLOBALS['post-carousel'] ) ) {
			...
			$output = '<ul>';
				...
				// Uncomment next two lines to show post categories
...
if ( !function_exists('ss_framework_portfolio_filter') ) {
	...
	function ss_framework_portfolio_filter() {
		...
		if( ss_framework_has_shortcode('portfolio') || is_tax('portfolio-categories') ) {
			...
			$output .= '<li class="all"><a data-categories="*">' . __('All', 'ss_framework') . '</a></li>';
...
if ( !function_exists('ss_framework_twitter_feed') ) {
	...
	function ss_framework_twitter_feed( $username, $tweet_count, $ignore_replies = true ) {
		...
		if ( $tweets === false  ) {
			...
			if ( !$tweet_found )
				$tweets = "<ul class='tweets-feed'><li>" . __('Oops, our Twitter feed is unavailable at the moment.', 'ss_framework') . " - <a href='http://twitter.com/$twitter_user_id/'>" . __('Follow us on Twitter!', 'ss_framework') . "</a></li></ul><!-- end .tweets-feed -->";
...
if ( !function_exists('ss_framework_flickr_feed') ) {
	...
	function ss_framework_flickr_feed( $flickr_id, $image_count ) {
		...
		if ( $images === false ) {
			...
			if( !is_wp_error( $rss_feed ) && isset( $rss_feed ) ) {
				...
				if( count( $image ) ) {
					...
					for( $i = 0; $i < $image_count; $i++ ) {
						...
						$images .= "<li><a href='$image_link' title='$image_title'><img src='$thumb' alt='$image_title'></a></li>\n";
			...
			if ( !$image_found ){
				$images = "<ul class='flickr-feed'><li>Oops, our Flickr feed is unavailable at the moment - <a href='http://flickr.com/$flickr_id/'>Check our images on Flickr!</a></li></ul><!-- end .flickr-feed -->";
			}
